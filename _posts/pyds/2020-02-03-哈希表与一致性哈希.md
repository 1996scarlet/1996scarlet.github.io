...
而与之相对应的链地址法则是通过开辟额外空间将发生冲突的数据

链地址法具有很强的灵活性, 其扩展结构并不局限于单向链表, 例如: JDK中的HashMap会根据每个节点所存储的冲突数据数量, 将扩展结构在链表和红黑树之间切换. 但缺点也很明显, 链表地址不连续, 随机寻址费劲, 插入删除容易; 链表遍历的时间复杂度O(n), 红黑树遍历时间复杂度O(logn)
