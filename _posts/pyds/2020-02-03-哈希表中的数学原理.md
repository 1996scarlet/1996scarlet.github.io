...
令$n$为字符串的数量, $k$为字符串的最大长度. 则遍历`strs`的时间复杂度为$O(n)$, 对字符串使用快速排序的时间复杂度为$O(k\log{k})$, 因此总体时间复杂度为$O(nk\log{k})$. 更多练习题请参考[探索哈希表](https://leetcode-cn.com/explore/learn/card/hash-table/)
